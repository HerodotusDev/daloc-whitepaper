...
The Harvest Vault's operator originates loans, responds to lifecycle events, and manages portfolio concentration---all within bounds set by a publicly auditable mandate and enforced per-epoch by a zero-knowledge proof. The vault is the canonical lending book: when it has sufficient idle capital, it funds loans directly at the full fixed rate (Section~\ref{sec:capital}); when demand exceeds capacity, it routes through external venues and commits only hedging reserves. Passive LPs deposit USDC and earn yield without trusting the operator's discretion---the mandate is the trust anchor, not reputation.
