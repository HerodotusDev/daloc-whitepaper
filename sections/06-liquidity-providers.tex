...
The Harvest Vault gives passive LPs pool-style UX while an operator deploys capital into loans under a publicly auditable mandate verified per-epoch by zero-knowledge proof.

\subsection{Operator and Mandate}

The operator's strategy is bounded by a \textbf{mandate}, a policy committed on-chain as a hash at vault deployment. The mandate specifies:
\begin{itemize}
...
The mandate constrains what the operator \emph{can} do, not what they \emph{must} do; within its bounds, the operator retains full discretion.

\subsection{Compliance Proof}

Each epoch, the operator produces a zero-knowledge proof that every action satisfied the mandate. The proof takes prior vault state, the epoch's actions, and the mandate hash as public inputs, outputting a \textbf{compliance fact} registered in the facts registry (Section~\ref{sec:settlement}):
\begin{equation}
  \begin{split}
...
LPs deposit USDC and receive ERC-20 shares. NAV equals idle USDC plus debt-token claim values plus escrowed hedging buffers plus accrued interest, less USDC ring-fenced for pending withdrawals. Share price is NAV per outstanding supply, with a virtual offset against inflation attacks. Yield comes from fixed-rate interest (vault-funded loans), buffer surplus (externally-sourced loans), and liquidation proceeds. Impairments reduce NAV directly via share-price adjustment.
